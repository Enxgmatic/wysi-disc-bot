...
async def skill_issue_call():
    ...
    now = timezone.localize(datetime.datetime.now())
    if not ((int(now.strftime("%H")) + 8) < 8 or (int(now.strftime("%H")) + 8) > 22):
        await bot.wait_until_ready()
        c = bot.get_channel(CHANNEL)
        USER_IDS = "<@" + USER_ID + ">"
...
async def on_message(message):
    ...
    elif message.content.lower() == "$wysi time":
        await message.channel.send(timezone.localize(datetime.datetime.now()))
